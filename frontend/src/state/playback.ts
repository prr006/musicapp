  ...
  private onEngineEvent(event: Parameters<Parameters<PlaybackEngine['subscribe']>[0]>[0]): void {
    switch (event.type) {
      ...
      case 'error':
        ...
        break
      case 'videoData': {
        // Video metadata from YouTube's getVideoData(). We intentionally do NOT
        // reconcile titles here: Piped search provides accurate metadata after
        // cleanYouTubeTitle() processing, and YouTube's getVideoData() can return
        // stale or mismatched data (especially on load). Title accuracy is
        // ensured at the parsePipedItem layer, not at playback reconciliation.
        break
      }
    }
  }
